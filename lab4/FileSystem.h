...
void FileSystem::print_file_info(std::string name, std::string owner) {
    ...
    for (auto it = f->sz.cbegin(); it != f->sz.cend(); it++) {
        ...
        std::cout << "  Content of '" << it->first << "' (" << it->second << " bytes):\t" << f->data[it->first] << std::endl;
    }
}
...
bool FileSystem::del_dir(std::string name, std::string own) {
    std::vector<std::string> out;
    std::pair<std::vector<std::string>, std::string> _pair = parser(name);
    ...
    std::string& dirname = _pair.second;
    Dir* d = find_dir(dirs, own);
    if (d != nullptr && d->elems.count(dirname) && d->elems[dirname]->user_rights[own].can_execute_or_move) {
        d->elems.erase(dirname);
    }
    return true;
}
