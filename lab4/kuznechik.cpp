...
char* kuz_encrypt(kuz_key_t* subkeys, char* from, uint32_t* len) {
	uint32_t len_out;
	if (*len % BLOCK_SIZE) {
		len_out = *len + BLOCK_SIZE - *len % BLOCK_SIZE;
	}
	else len_out = *len;
	...
	memcpy(out, from, *len);
	*len = len_out;
	for (uint16_t i = 0; i < len_out / BLOCK_SIZE; i++) {
		kuz_encrypt_block(subkeys, out + i * BLOCK_SIZE);
	}
	return out;
}
...
char* kuz_decrypt(kuz_key_t* subkeys, char* from, uint32_t len) {
	...
	memcpy(out, from, len);
	for (uint16_t i = 0; i < len_out / BLOCK_SIZE; i++) {
		kuz_decrypt_block(subkeys, out + i * BLOCK_SIZE);
	}
	return out;
}
...
char* kuz_encrypt_ctr(kuz_key_t* subkeys, char* from, uint32_t len) {
	...
	for (uint16_t i = 0; i < len_out / BLOCK_SIZE; i++) {
		...
		*(uint16_t*)ctr = i;
		kuz_encrypt_block(subkeys, ctr);
		for (uint16_t j = 0; j < BLOCK_SIZE; j++)
			out[BLOCK_SIZE * i + j] ^= ctr[j];
	}
	return out;
}
